#ifndef MacanumCalculation_HPP
#define MacanumCalculation_HPP
...
namespace Alg::CalculationBase
{
    /**
     * @class Macanum_FK
     * @brief 麦克纳姆轮底盘正向运动学计算类
     * 
     * 根据四个麦克纳姆轮的转速计算底盘的运动状态（速度和角速度）
     * 继承自ForwardKinematicsBase类
     */
    class Macanum_FK : public ForwardKinematicsBase
    {
        public:
            /**
             * @brief 构造函数
             ...
             */
            Macanum_FK(float r = 1.0f, float s = 1.0f) 
             ...
             * 先设置轮子转速，然后执行正向运动学计算
             */            
            void MacanumForKinematics(float w0, float w1, float w2, float w3)
            {
    ...
    /**
     * @class Macanum_ID
     * @brief 麦克纳姆轮底盘逆向动力学计算类
     * 
     * 根据底盘受到的外力和力矩计算每个轮子需要产生的扭矩
     * 继承自InverseDynamicsBase类
     */
    class Macanum_ID : public InverseDynamicsBase
    {
        public:
            /**
             * @brief 构造函数
             ...
             * @param s 轮子半径
             */
            Macanum_ID(float r = 1.0f, float s = 1.0f) 
                : R(r), S(s)
            {
             ...
             */
            void MacanumInvDynamics(float fx, float fy, float torque)
            {
    ...
    /**
     * @class Macanum_IK
     * @brief 麦克纳姆轮底盘逆向运动学计算类
     * 
     * 根据期望的底盘运动状态计算每个轮子的目标速度
     * 继承自InverseKinematicsBase类
     */
    class Macanum_IK : public InverseKinematicsBase
    {
        public:
            /**
             * @brief 构造函数
             ...
             * @param s 轮子半径
             */
            Macanum_IK(float r = 1.0f, float s = 1.0f) 
                : R(r), S(s) 
            {
                sqrt2_2 = 1.414f / 2.0f;
                for(int i = 0; i < 4; i++)
             ...
             */
            void MacanumInvKinematics(float vx, float vy, float vw, float phase, float speed_gain)
            {
                SetPhase(phase);
